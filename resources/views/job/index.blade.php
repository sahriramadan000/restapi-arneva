        ...
        <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                <div class="bg-gradient-primary shadow-primary border-radius-lg pt-3 pb-3 d-flex align-items-center justify-content-between">
                    ...
                    <a href="{{ route('job.create') }}" class="btn btn-sm bg-success text-white me-3 mb-0">Add Data</a>
                </div>
            </div>
                    ...
                    <table class="table align-items-center mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2" width="15%">Job Code</th>
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Job Name</th>
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Job Detail</th>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.code === 404) {
                        var noDataMessage = `<tr><td colspan="5" class="text-center">Data not found</td></tr>`;
                        $('#data-job').append(noDataMessage);
                    } else {
                        $.each(response.data, function(index, dt) {
                            var editUrl = "{{ route('job.edit', ':id') }}".replace(':id', dt.id);
                            var deleteUrl = `${globalURL}/deleteJob/${dt.id}`;
                            var addList = `<tr>`+
                                                ...
                                                `<td><span class="text-secondary text-xs font-weight-bold align-middle">${dt.detail}</span></td>`+
                                                `<td>
